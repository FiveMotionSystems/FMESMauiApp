...
namespace FMES
{
    class clsPartsList
    {
        ...
        //public clsKaisouHeader _Header;
        public List<clsPartsData> _Datas = new List<clsPartsData>();


        public bool GetList(int wUserID, string wSasizuNo, int wKaisouNo, int wKouteiID, int wKouteiShousaiID, int wKensaBui, int whinnmokuID, int wlastMode, int wVer, ref string srtErrMsg)
        {
            bool bRet = false;
            int iRow = 0;
            string wURL = clsGlobalVar.GetCurURL() + "users/getlots/" + clsGlobalVar.GetLanguageStr() + "/" + wSasizuNo + "/" + whinnmokuID + "/" + wKouteiID;
            ...
            if (string.IsNullOrEmpty(strRet) == false)
            {
                if (strRet.IndexOf("NG:") == -1)
                {
                    ...
                    foreach (string wStr in lstRec)
                    {
                        ...
                        {
                            clsPartsData wPartsData = new clsPartsData(wStr);
                            _Datas.Add(wPartsData);
                        }
                        iRow += 1;
                    }
                    lstRec.Clear();
