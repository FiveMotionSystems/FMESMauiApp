...
namespace FMESSignage
{
     class clsSiteList
    {
        ...
        public List<clsSiteData> _Datas = new List<clsSiteData>();
        public bool GetList(string wUID, string wpasswd, ref string srtErrMsg)
        {
            bool bRet = false;
            int iRow = 0;
            //string wURL = clsGlobalVar.GetCurURL() + "users/tabgetplace/" + clsGlobalVar.GetLanguageStr() + "/" + wUID + "/" + wpasswd;
            string strSend = clsGlobalVar.GetCurURL() + "users/tabgetplace/" + wUID + "/" + wpasswd;

            string strErr = null;
            SendLog(strSend, ref strErr);
            ...
            if (string.IsNullOrEmpty(strRet) == false)
            {
                if (strRet.IndexOf("NG:") == -1)
                {
                    ...
                    foreach (string wStr in lstRec)
                    {
                        ...
                        {
                            clsSiteData wSiteData = new clsSiteData(wStr);
